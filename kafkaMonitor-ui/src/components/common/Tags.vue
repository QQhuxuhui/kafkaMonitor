<template>
  <div class="tags" v-if="showTags">
    <ul>
      <li class="tags-li" v-for="(item,index) in this.$store.state.tagsList" :class="{'active': isActive(item.path)}"
          :key="index">
...
<script>
  export default {
    ...
    methods: {
      ...
      // 关闭其他标签
      closeOther() {
        const curItem = this.$store.state.tagsList.filter(item => {
          return item.path === this.$route.fullPath;
        });
        this.$store.state.tagsList = curItem;
      },
// 设置标签
      setTags(route) {
        ...
        if (!isExist) {
          if (this.$store.state.tagsList.length >= 8) {
            this.$store.state.tagsList.shift();
          }
          this.$store.state.tagsList.push({
